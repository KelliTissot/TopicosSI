...
def train_agent(env, use_sarsa, num_episodes=NUM_EPISODES):
    ...
    for episode in range(num_episodes):
        ...
        while True:
            ...
            # Visualize the environment every 10 episodes
            if episode % 10 == 0:
                screen.fill(WHITE)
                draw_grid(screen, env)
                pygame.display.flip()
                pygame.time.delay(100)  # Delay to see the update (in milliseconds)
...
def main():
    global screen  # Make the screen variable global for access in train_agent
    ...
    env = GridWorld()  # Reset environment
